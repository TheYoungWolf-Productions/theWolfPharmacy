...
$stmt = $conn->prepare("select products.id, products.name, batch, import_date, expiry_date, quantity, suppliers.name as 'supplier' from products, inventory, suppliers where products.id=inventory.product_id and quantity<30 and supplier_id=suppliers.id order by quantity");
$stmt->execute();
