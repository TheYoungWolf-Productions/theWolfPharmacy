...
$stmt = $conn->prepare("select products.id, products.name, batch, import_date, expiry_date, quantity, suppliers.name as 'supplier' from products, inventory, suppliers where products.id=inventory.product_id and supplier_id=suppliers.id and DATEDIFF(expiry_date, CURDATE()) < 35  order by expiry_date");
$stmt->execute();
